...
class PostsController extends Controller
{
    ...
    public function showPost($id_blog, $id)
    {
        ...
        if (!$post) {
            return redirect()->route('foro', [], 301)->with('error', 'Post no encontrado');
        }
        ...
        if (!$metaTags) {
            $metaTags = new MetaTag([
                'page' => 'foro',
                'meta_title' => 'Foro - Descubre las últimas novedades',
                'meta_description' => 'Explora artículos interesantes sobre diversos temas en nuestro foro.',
                'meta_keywords' => 'foro, artículos, novedades',
                'meta_robots' => 'index, follow',
                'canonical_url' => url()->current(),
            ]);
        }
        
        $metaTags->meta_title = $post->titulo;

        $metaTags->meta_title = 'OnlyEscorts | Foro - ' . $post->titulo;
        $metaTags->canonical_url = url()->current();

        return view('showcomentario', compact('post', 'categoria', 'ciudades', 'comentarios', 'foro', 'sectores', 'nacionalidades', 'metaTags'));
    }
