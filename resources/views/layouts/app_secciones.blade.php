...
<head>
    ...
    <title>@yield('title', 'OnlyEscorts')</title>

    <meta name="description" content="@yield('description', 'Encuentra anuncios eróticos de Escorts, Damas de compañía y Masajes Eróticos en todo Chile en el sitio más confiable. En OnlyEscorts descubrirás las mejores mujeres para experimentar el mejor Sexo en Chile.  Contacta directamente por Whatsapp!')">
...
<body class="login-page">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.removeExistingVideo = function(video, button) {
                ...
                fetch('/usuarios-publicate/eliminar-video', {
                    method: 'POST',
                    headers: {
...
tinymce.init({
    selector: '#contenido',
    selector: '.tinyenlace',
    plugins: 'emoticons lists media link',
    toolbar: [
        'undo redo | blocks | formatselect | bold italic underline',
        'alignleft aligncenter alignright | bullist numlist | emoticons | media link unlink'
    ],
    menubar: false,
    height: 300,
    forced_root_block: 'p',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function manejarEnvioFormulario(e, tipo) {
        ...
        const ciudadId = ciudadSelect.value;
        if (!id && tipo !== 'disponibilidad' && tipo !== 'resenas' & tipo !== 'general') {
...
<script>
    const routeUrls = {
        'general': {
            'update': "{{ route('secciones.general.update') }}"
        },
        'servicio': {
            'get': "{{ route('api.servicios.secciones', ['servicio' => ':id']) }}",
            'update': "{{ route('secciones.servicios.update') }}"
        },
        'atributo': {
            'get': "{{ route('api.atributos.secciones', ['atributo' => ':id']) }}",
            'update': "{{ route('secciones.atributos.update') }}"
        },
        'nacionalidad': {
            'get': "{{ route('api.nacionalidades.secciones', ['nacionalidad' => ':id']) }}",
            'update': "{{ route('secciones.nacionalidades.update') }}"
        },
        'sector': {
            'get': "{{ route('api.sectores.secciones', ['sector' => ':id']) }}",
            'update': "{{ route('secciones.sectores.update') }}"
        },
        'categoria': {
            'get': "{{ route('api.categorias.secciones', ['categoria' => ':id']) }}",
            'update': "{{ route('secciones.categorias.update') }}"
        },
        'disponibilidad': {
            'get': "{{ route('api.disponibilidad.secciones') }}",
            'update': "{{ route('secciones.disponibilidad.update') }}"
        },
        'resenas': {
            'get': "{{ route('api.resenas.secciones') }}",
            'update': "{{ route('secciones.resenas.update') }}"
        }
    };
</script>
</html>